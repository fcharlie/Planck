...
ssize_t MagicIndex(memview mv, size_t offset) {
  ...
  return reinterpret_cast<const char *>(p) - mv.data();
}

status_t inquisitive_msxmldocs(memview mv, inquisitive_result_t &ir) {
  constexpr const byte_t docsMagic[] = {'P', 'K', 0x03, 0x04};
  ...
  ssize_t startoffset = planck::readle<uint32_t>(mv.data() + 18) + 49;
