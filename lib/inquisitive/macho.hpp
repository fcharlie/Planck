...
#define MH_NOUNDEFS                                                            \
  0x1 /* the object file has no undefined \ references */
#define MH_INCRLINK                                                            \
...
#define MH_DYLDLINK                                                            \
  0x4 /* the object file is input for the \                                                                             \
         dynamic linker and can't be staticly \ link edited again */
#define MH_BINDATLOAD                                                          \
  0x8 /* the object file's undefined \                                                                             \
         references are bound by the dynamic \ linker when loaded. */
#define MH_PREBOUND                                                            \
  0x10 /* the file has its dynamic undefined \ references prebound. */
#define MH_SPLIT_SEGS                                                          \
  0x20 /* the file has its read-only and \ read-write segments split */
#define MH_LAZY_INIT                                                           \
  0x40 /* the shared library init routine is \                                                                             \
          to be run lazily via catching memory \                                                                             \
          faults to its writeable segments \ (obsolete) */
#define MH_TWOLEVEL                                                            \
  0x80 /* the image is using two-level name \ space bindings */
...
#define MH_CANONICAL                                                           \
  0x4000 /* the binary has been canonicalized \ via the unprebind operation */
#define MH_WEAK_DEFINES                                                        \
  0x8000 /* the final linked image contains \ external weak symbols */
#define MH_BINDS_TO_WEAK                                                       \
  0x10000 /* the final linked image uses \ weak symbols */
...
#define MH_ROOT_SAFE                                                           \
  0x40000 /* When this bit is set, the binary \                                                                             \
             declares it is safe for use in \ processes with uid zero */
...
#define SG_FVMLIB                                                              \
  0x2 /* this segment is the VM that is allocated by \                                                                             \
         a fixed VM library, for overlap checking in \ the link editor */
#define SG_NORELOC                                                             \
...
#define S_NON_LAZY_SYMBOL_POINTERS                                             \
  0x6 /* section with only non-lazy \ symbol pointers */
#define S_LAZY_SYMBOL_POINTERS                                                 \
  0x7 /* section with only lazy symbol \ pointers */
#define S_SYMBOL_STUBS                                                         \
  0x8 /* section with only symbol \                                                                             \
         stubs, byte size of stub in \ the reserved2 field */
#define S_MOD_INIT_FUNC_POINTERS                                               \
  0x9 /* section with only function \ pointers for initialization*/
#define S_MOD_TERM_FUNC_POINTERS                                               \
  0xa /* section with only function \ pointers for termination */
#define S_COALESCED                                                            \
  0xb /* section contains symbols that \ are to be coalesced */
...
#define S_INTERPOSING                                                          \
  0xd /* section with only pairs of \                                                                             \
         function pointers for \ interposing */
#define S_16BYTE_LITERALS                                                      \
  0xe /* section with only 16 byte \ literals */
#define S_DTRACE_DOF                                                           \
  0xf /* section contains \ DTrace Object Format */
#define S_LAZY_DYLIB_SYMBOL_POINTERS                                           \
  0x10 /* section with only lazy \                                                                             \
          symbol pointers to lazy \ loaded dylibs */
/*
...
#define S_ATTR_LIVE_SUPPORT                                                    \
  0x08000000 /* blocks are live if they \ reference live blocks */
#define S_ATTR_SELF_MODIFYING_CODE                                             \
  0x04000000 /* Used with i386 code stubs \ written on by dyld */
/*
...
#define S_ATTR_SOME_INSTRUCTIONS                                               \
  0x00000400 /* section contains some \ machine instructions */
#define S_ATTR_EXT_RELOC                                                       \
  0x00000200 /* section has external \ relocation entries */
